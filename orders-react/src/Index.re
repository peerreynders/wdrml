/* file: src/Index.re
   ...
   Original:
   https://media.pragprog.com/titles/reasonml/code/reason-react/shirt-react/src/Index.re
 ...
 */
%bs.raw
  "require('./styles.css');"; /* for webpack */
...
let withEdit:
  ...
  (~state as {edit: {qty, size, sleeve, color, pattern}}, ~dispatch) =>
    <OrderEditView
      key="orderEditView"
      qty
      size
...
let withView:
  (~state: State.t, ~dispatch: Action.t => unit) => ReasonReact.reactElement =
  (~state as {orders}, ~dispatch) => {
    let onOrderDelete = makeDeleteListener(dispatch);

    <OrdersView key="ordersView" orders onOrderDelete />;
  };
