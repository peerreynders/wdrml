...
let makeOptionElement: string => Rr.reactElement =
  value => {
    let key = value;
    let name = Rr.string(value);
...
let makeSelect:
  ...
  (_id, name, choices, value, onChange) => {
    ...
    let options = Belt.Array.map(choices, makeOptionElement)->Rr.array;
...
let make:
  ...
  (
    ...
    let retainedProps = {qty, size, sleeve, color, pattern};
    ...
    let render: s => Rr.reactElement =
      _self => {
        ...
        <p className="flex">
          ...
          patternSelect
          <span className="item">
            <button onClick> {Rr.string("Add")} </button>
          </span>
        </p>;
      };
