...
let initialState = () => Storage.getStoredState();
let putStoredState: s => unit = ({state}) => Storage.putStoredState(state);

let reducer: (Action.t, State.t) => u =
  (action, state) =>
    switch (action) {
    | CommitOrder =>
      Rr.UpdateWithSideEffects(commitOrder(state), putStoredState)
    ...
    | Delete(orderNumber) =>
      Rr.UpdateWithSideEffects(
        deleteOrder(orderNumber, state),
        putStoredState,
      )
    | Edit(orderNumber) =>
      Rr.UpdateWithSideEffects(editOrder(orderNumber, state), putStoredState)
    };
